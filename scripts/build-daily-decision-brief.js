...
function main(){
  ...
  const opportunities=scored.filter(r=>sclass(r)!=="risk").sort((a,b)=>priorityScore(b)-priorityScore(a)||targetProbability(b)-targetProbability(a)||expectedReturnPct(b)-expectedReturnPct(a)).slice(0,10).map(r=>({symbol:r.symbol,name:r.name_ar||r.name_en||r.name||"",confidence:r._conf,rawConfidence:r._rawConf,sessions:r._sessions,price:num(r.price),entryFrom:num(r.entryFrom),entryTo:num(r.entryTo),target1:num(r.target1),target2:num(r.target2),stopLoss:num(r.stopLoss),rr:Number(r._rr.toFixed(2)),targetProbability:targetProbability(r),potentialProfitPct:Number(potentialProfitPct(r).toFixed(2)),expectedReturnPct:Number(expectedReturnPct(r).toFixed(2)),priorityScore:priorityScore(r),reason:r.reason||""}));
