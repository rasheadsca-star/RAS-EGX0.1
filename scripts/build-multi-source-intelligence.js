#!/usr/bin/env node
/* EGX Pro Hub V9.8 — Build Source Evidence Matrix from Coverage Loop */
...
function decision(r){const score=num(r.sourceStrengthScore||r.evidenceScore)||0; if(r.coverageStatus==='ANALYSIS_READY'&&score>=75)return 'TRUSTED_ANALYSIS_READY'; if(r.coverageStatus==='ANALYSIS_READY')return 'ANALYSIS_READY'; if(r.coverageStatus==='WATCH_NEEDS_EVIDENCE')return 'WATCH_NEEDS_EVIDENCE'; return 'BLOCKED_MISSING_CORE_DATA'}
function main(){
 const coverage=read('data/evidence-coverage-loop-status.json',{}); const base=mapBase();
 let rows=rowsOf(coverage).map(x=>{const b=base[sym(x.symbol)]||{}; const conf=Array.isArray(x.confirmationSources)?x.confirmationSources:[]; const d=decision(x); return {...x,name:x.name||b.name_ar||b.name||'',price:b.price,volume:b.volume,valueTraded:b.valueTraded,sector:b.sector||b.sector_ar||'غير مصنف',finalDataDecision:d,decision:d,level:d,sourceStrengthScore:num(x.sourceStrengthScore)||0,evidenceScore:num(x.evidenceScore||x.sourceStrengthScore)||0,confirmationCount:num(x.confirmationCount)||conf.length,confirmationSources:conf,sourceQuality:{price:x.priceStatus,volume:x.volumeStatus,support:x.supportStatus,plan:x.planStatus},reason:x.exclusionReason||''}});
 const total=coverage.summary?.totalUniverse || Object.keys(base).length || rows.length; const ready=rows.filter(x=>/READY|TRUSTED/.test(x.finalDataDecision)).length;
 const summary={...(coverage.summary||{}),totalUniverse:total,analysisReadySymbols:ready,reliableSymbols:ready,reliableCoveragePct:Number((ready/Math.max(1,total)*100).toFixed(1)),targetCoveragePct:coverage.summary?.targetCoveragePct||80,avgSourceStrength:Math.round(rows.reduce((s,x)=>s+(num(x.sourceStrengthScore)||0),0)/Math.max(1,rows.length)),avgConfirmations:Number((rows.reduce((s,x)=>s+(num(x.confirmationCount)||0),0)/Math.max(1,rows.length)).toFixed(1))};
 rows=rows.sort((a,b)=>(num(b.sourceStrengthScore)-num(a.sourceStrengthScore))||a.symbol.localeCompare(b.symbol));
 const out={ok:summary.reliableCoveragePct>=summary.targetCoveragePct,engine:'v9_8_source_evidence_matrix_builder',generatedAt:RUN_AT,summary,rows,note:'Evidence matrix generated from iterative coverage loop; analysis-ready does not equal execution permission.'};
 write('data/source-evidence-matrix.json',out); write('data/multi-source-intelligence.json',out); console.log('V9.8 evidence matrix', summary);
}
main();
