...
function main(){
  const source=read("data/source-health.json",{}), fetchVerify=read("data/workflow-source-verification.json",{}), fetchStatus=read("data/fetch-status.json",{}), sourceFetchReport=read("data/source-fetch-report.json",{}), gateway=read("data/source-gateway-report.json",{}), sourceAlerts=read("data/source-alerts.json",{}), price=read("data/price-reconciliation-report.json",{summary:{}}), hist=read("data/history-backfill-plan.json",{}), ranking=read("data/final-opportunity-ranking.json",{summary:{}}), alerts=read("data/alert-decision-center.json",{summary:{}}), reg=read("data/app-regression-report.json",{});
  const checks=[];
  const coverage=num(source.universeCoveragePct||source.coveragePct||0);
  checks.push({id:"fetcher",name:"جلب البيانات",state:(gateway.accepted||fetchStatus.realFetch||fetchVerify.realFetch||sourceFetchReport.realFetch)?"ok":gateway.fallbackUsed?"warn":"warn",value:gateway.status||fetchStatus.mode||fetchVerify.fetchMode||"unknown",message:gateway.accepted?`بوابة البيانات قبلت ${gateway.marketRows||0} صف`:gateway.fallbackUsed?"وضع Degraded: استخدام آخر Snapshot ناجح":(fetchStatus.realFetch||fetchVerify.realFetch||sourceFetchReport.realFetch)?"الجلب العام/الخارجي مضبوط":"لا يوجد جلب خارجي مؤكد؛ قد تكون البيانات من ملفات حالية",action:(fetchStatus.realFetch||fetchVerify.realFetch)?"متابعة":"افتح تحقق الجلب واضبط EGX_MARKET_JSON_URL"});
