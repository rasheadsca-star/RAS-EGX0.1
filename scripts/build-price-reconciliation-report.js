...
function main(){
 ...
 const rows=symbols.map(s=>{
  ...
  return {symbol:s,name:rr?.name_ar||rr?.name_en||rr?.name||cr?.name_ar||cr?.name_en||cr?.name||mr?.name_ar||mr?.name_en||mr?.name||"",finalPrice:chosen.price||price(rr)||price(cr)||price(mr)||hp,recommendationPrice:price(rr),marketPrice:price(mr),cachePrice:price(cr),historyPrice:hp,sourceUsed:chosen.source||"fallback",sourceTimestamp:chosen.ts||null,sourceAgeMinutes:ag==null?null:Number(ag.toFixed(1)),isStale:ag!=null?ag>180:false,hasConflict:diff>=0.75,diffPct:Number(diff.toFixed(2)),conflictSummary:diff>=0.75?`max/min internal price spread ${diff.toFixed(2)}%`:"",candidates:cand};
 });
 const summary={total:rows.length,ok:rows.filter(x=>!x.isStale&&!x.hasConflict).length,stale:rows.filter(x=>x.isStale).length,conflict:rows.filter(x=>x.hasConflict).length,adjusted:rows.filter(x=>x.finalPrice&&x.recommendationPrice&&Math.abs(x.finalPrice-x.recommendationPrice)>0.0001).length};
 const last=source.generatedAt||source.lastSuccessAt||market.updatedAt||rec.generatedAt||null;
 write("data/price-reconciliation-report.json",{ok:true,engine:"v9_3_price_reconciliation",generatedAt:new Date().toISOString(),lastSourceUpdate:last,sourceAgeMinutes:age(last)==null?null:Number(age(last).toFixed(1)),summary,rows,note:"Final price is selected from internal public/delayed sources. This is not tick-by-tick live pricing."});
